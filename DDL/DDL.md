...
- ### Create
  ...
  - `CREATE TABLE company("id" INT GENERATED ALWAYS AS IDENTITY, "name" VARCHAR(128) UNIQUE NOT NULL, manager VARCHAR(128) REFERENCES employee (id), "date" DATE NOT NULL CHECK (date > '1995-01-01' AND date < '2020-01-01'), PRIMARY KEY (id) , UNIQUE(name, date));` - создание новой таблицы с полями в БД
     ...
     - `REFERENCES` - указатель на колонку строннюю таьлицу
- ### Drop
