...
- ### Create
  ...
  - `CREATE TABLE company("id" INT GENERATED ALWAYS AS IDENTITY, "name" VARCHAR(128) UNIQUE NOT NULL DEFAULT 'Sandnes', manager VARCHAR(128) REFERENCES employee (id), "date" DATE NOT NULL CHECK (date > '1995-01-01' AND date < '2020-01-01'), PRIMARY KEY (id) , UNIQUE(name, date), FOREIGN KEY (company_name) REFERENCES company(name) ON DELETE CASCADE));` - создание новой таблицы с полями в БД
     ...
     - `DEFAULT "..."` - задать дефолтное значение
...
- ### Triggers
   - `CREATE TABLE audit(id INT, table_name TEXT, date TIMESTAMP);` - триггеры нужны чтобы по sql запросам в одну таблицу делать transaction в другую
   - `CREATE FUNCTION audit_function() returns TRIGGER LANGUAGE plpsql AS $$ begin insert into audit (id, table_name, date) values(new.id, tg_table_name, now(); return null); end; $$;`
   - `CREATE TRIGGER audit_aircraft_trigger AFTER UPDATE OR INSERT OR DELETE ON aircraft FOR EACH ROW EXECUTE FUNCTION audit_function;`
   - `insert into aircraft(model) values('новый боинг)` - вызовется триггерная функция и добавить данные в audit
